...
/// from seedling to a parachain runtime that implements Aura.
		...
		|client,
		 ...
		 force_authoring| {
			...
			let aura_consensus = BuildOnAccess::Uninitialized(Some(Box::new(move || {
				...
				AuraConsensus::build::<<AuraId as AppCrypto>::Pair, _, _, _, _, _, _>(BuildAuraConsensusParams {
					proposer_factory,
...
/// Checks that the hardware meets the requirements and print a warning
/// otherwise.
fn warn_if_slow_hardware(hwbench: &sc_sysinfo::HwBench) {
	// Polkadot para-chains should generally use these requirements to ensure that
	// the relay-chain will not take longer than expected to import its blocks.
