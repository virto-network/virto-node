...
.PHONY: benchmark
benchmark:
	./target/release/virto-parachain benchmark pallet --chain=dev --execution=wasm --wasm-execution compiled --extrinsic="*" --pallet=$(pallet) --steps=20 --repeat=10 --heap-pages=4096 --output ./pallets/communities/src/weights.rs --template ./.maintain/frame-weight-template.hbs