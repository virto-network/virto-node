...
.PHONY: benchmark
benchmark:
	cargo run --release --features=runtime-benchmarks -- benchmark --chain dev  --execution=wasm --wasm-execution compiled --extrinsic="*" --pallet=$(pallet) --steps=20 --repeat=10 --heap-pages=4096 --output ./runtime/src/weights/payments.rs --template ./.maintain/frame-weight-template.hbs