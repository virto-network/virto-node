...
# NOTE: leaving out for now as we won't be using it for our deployment
...
# COPY --from=builder \
#     /virto/target/release/virto-node /usr/bin
...
COPY --from=builder \
    /virto/target/release/wbuild/virto-runtime/virto_runtime.wasm \
    /var/opt/
CMD ["cp", "-v", "/var/opt/virto_runtime.wasm", "/runtime/"]

FROM debian:bullseye-slim
COPY --from=builder \
    /virto/target/release/virto-node /usr/bin

CMD ["/usr/bin/virto-node"]
