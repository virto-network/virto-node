...
impl<
	...
	> TransactAsset
	for MultiCurrencyAdapter<Mutate, UnknownAsset, Match, AccountId, AccountIdConvert, CurrencyId, CurrencyIdConvert>
{
	...
	fn withdraw_asset(asset: &MultiAsset, location: &MultiLocation) -> result::Result<Assets, XcmError> {
		...
		// 		.saturated_into();
        //         // Mutate::burn_from(currency_id, &who, amount).map_err(|e| XcmError::FailedToTransactAsset(e.into()))
		// })?;
