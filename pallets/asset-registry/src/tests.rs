...
mod register_reserve_assest {
	...
	fn valid_locations_succced() {
		let native_frame_based_currency = MultiLocation {
			parents: 1,
			interior: X2(Parachain(1000), PalletInstance(1)),
		};
		let multiasset_pallet_instance = MultiLocation {
			parents: 1,
			interior: X3(Parachain(1000), PalletInstance(1), GeneralIndex(2)),
		};
		let relay_native_currency = MultiLocation {
			parents: 1,
	...
	fn invalid_locations_fail() {
		...
		let invalid_general_index = MultiLocation {
			parents: 1,
			interior: X2(Parachain(1000), GeneralIndex(1u128)),
		};

		new_test_ext().execute_with(|| {
			assert_noop!(
				AssetRegistry::register_reserve_asset(RuntimeOrigin::root(), LOCAL_ASSET_ID, governance_location,),
				Error::<Test>::WrongMultiLocation
			);

			assert_noop!(
				AssetRegistry::register_reserve_asset(RuntimeOrigin::root(), LOCAL_ASSET_ID, invalid_general_index,),
				Error::<Test>::WrongMultiLocation
			);
		})
	}
