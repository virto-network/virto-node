...
benchmarks! {
	...
	create {
		...
		let _ = T::Asset::deposit(1u32, &caller, 100u32);
		let recipent = account("recipient", 0, SEED);
	}: _(RawOrigin::Signed(caller.clone()), recipent, 1u32, 80u32)
	verify {
		assert_last_event::<T>(Event::<T>::PaymentCreated(caller, 1u32, 2u32).into());
	}
	...
	release {
		...
		Payment::<T>::create(RawOrigin::Signed(caller.clone()).into(), recipent.clone(), 1u32, 80u32)?;
	...
	resolve {
		...
		let recipent : T::AccountId = account("recipient", 0, SEED);
		Payment::<T>::create(RawOrigin::Signed(caller.clone()).into(), recipent.clone(), 1u32, 80u32)?;
	}: _(RawOrigin::Signed(caller.clone()), caller.clone(), recipent.clone(), PaymentState::Cancelled)
	verify {
		assert_last_event::<T>(Event::<T>::PaymentCancelled(caller, recipent).into());
	}
}
